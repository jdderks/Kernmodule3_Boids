...
/// <summary>
/// Made with help from tutorials by Daniel Shiffman (the Coding Train) 
/// </summary>
public class Flocking : MonoBehaviour
{
    ...
    [Range(1, 100)]
    public int sceneWidth = 14;

    [Range(1, 100)]
    public int sceneHeight = 8;
    ...
    {
        if (Position.x < -sceneWidth) Position = new Vector2(sceneWidth, Position.y);
        if (Position.y < -sceneHeight) Position = new Vector2(Position.x, sceneHeight);
        if (Position.x > sceneWidth) Position = new Vector2(-sceneWidth, Position.y);
        if (Position.y > sceneHeight) Position = new Vector2(Position.x, -sceneHeight);
    }
}